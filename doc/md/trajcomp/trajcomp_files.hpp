...
size_t loadXY(std::string fname, ttraj &s)
{
	s.clear();
	std::ifstream infile(fname);
	if (!infile)
...
// ------------------ START JANS IMPLEMENTATION -----------------------------
/*
* Loads time properties of trajectory
*/
template<typename timeTraj>
size_t loadTime(std::string fname, timeTraj &s)
{
	s.clear();
	std::ifstream infile(fname);
	if (!infile)
